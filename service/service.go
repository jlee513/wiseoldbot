...
func (s *Service) StartDiscordIRC() {
	s.sheets.InitializeSubmissionsFromSheet(s.config.SheetsCp, s.submissions)
	...
	go func() {
		...
		s.sheets.UpdateCpSheet(s.config.SheetsCp, s.submissions)
		s.sheets.UpdateCpScreenshotsSheet(s.config.SheetsCpSC, s.cpscreenshots)
	}()
...
func (s *Service) updateMemberList(ctx context.Context, session *discordgo.Session, message *discordgo.MessageCreate) {
	...
	if re.Match([]byte(message.Content)) {
		...
		if s.userExists(session, member, message.ChannelID) {
...
func (s *Service) userExists(session *discordgo.Session, member string, channelID string) (exists bool) {
	exists = false
